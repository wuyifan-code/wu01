      ...
      <div class="max-w-4xl mx-auto text-center relative z-10">
        ...
        <div class="flex flex-wrap justify-center gap-4">
          ...
          <a href="#ecosystem" class="px-6 py-3 bg-ecosystem text-white rounded-lg font-medium hover:bg-ecosystem/90 transition-all shadow-lg hover:shadow-xl btn-bounce">
            生态路线 <i class="fa fa-arrow-right ml-2"></i>
          </a>
        ...
        <div class="overflow-x-auto">
          <table class="w-full bg-white">
            ...
            <tbody>
              ...
              <tr class="border-b hover:bg-gray-50 transition-colors duration-300">
                <td class="py-4 px-6 font-medium">用户留存率（30天）</td>
                  ...
                  <div class="bg-gray-50 p-5 rounded-xl mt-4">
                    ...
                    <ul class="space-y-3">
                      ...
                      <li class="flex items-start">
                        ...
                        <div class="ml-3">
                          <p class="font-medium text-gray-800">客户响应不及时</p>
                    ...
                    <ul class="space-y-3">
                      ...
                      <li class="flex items-start">
                        ...
                        <div class="ml-3">
                          ...
                          <p class="text-sm text-gray-600">约35%工作时间用于文书和咨询回复</p>
                        </div>
                      </li>
                    </ul>
                  </div>
  ...
  <script>
    ...
    modalTabs.forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        modal.querySelectorAll('.modal-panel').forEach(panel => {
    ...
    window.addEventListener('load', () => {
      ...
      new Chart(overviewCtx, {
        type: 'bar',
        data: {
          labels: ['月活用户（百万）', '企业客户（万）', 'API调用成本（相对值）', '技术性能（%）'],
          datasets: [
            {
              label: 'DeepSeek（普惠）',
              data: [12, 8.2, 1, 83.5],
    ...
    const progressObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          setTimeout(() => {
            entry.target.style.width = entry.target.style.height === '100%' ? '100%' : 
                                     entry.target.style.height === '25%' ? '25%' :
                                     entry.target.style.height === '94%' ? '94%' :
                                     entry.target.style.height === '97%' ? '97%' :
                                     entry.target.style.height === '93%' ? '93%' :
                                     entry.target.style.height === '98%' ? '98%' : '100%';
          }, 100);
    ...
    <!-- 三级模态框：AI客服聊天窗口 -->
    <div id="ai-chat-modal" class="modal fixed inset-0 flex items-center justify-center bg-black/50 modal-backdrop">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            chatButton.innerHTML = '<i class="fa fa-comments text-xl"></i>';
            ...
            closeChat.addEventListener('click', hideModal);
                ...
                keywords: [
                    {
                        keys: ["退款", "退钱", "返还", "退费"],
                        replies: [
                            "关于退款问题，麻烦告诉我你的订单号，我帮你查进度~",
                            "退款需要先核实订单状态哦，你方便提供下单手机号或订单号吗？",
                            "好的，退款流程一般1-3个工作日，我先帮你确认是否已提交申请"
                        ]
                    },
                    {
                        keys: ["物流", "快递", "发货", "运输", "配送"],
                        replies: [
                            "稍等！我这就帮你查物流轨迹，你订单号是多少呀？",
                            "物流信息可能有延迟，告诉我订单号，我帮你催一下快递小哥~",
                            "发货后一般2-4天送达，你订单号多少？我查下具体位置"
                        ]
                    },
                    {
                        keys: ["产品", "故障", "坏了", "无法使用", "质量", "损坏"],
                        replies: [
                            "别担心！你先描述下产品具体的问题现象（比如报错/没反应），我帮你排查~",
                            "产品故障可以先试试重启哦，要是不行的话，告诉我你的购买时间和型号",
                            "质量问题我们会负责到底的，你先拍张问题照片发给我可以吗？"
                        ]
                    },
                    {
                        keys: ["咨询", "问题", "了解", "想知道", "询问"],
                        replies: [
                            "当然可以！你具体想了解哪方面呢？比如价格/功能/售后~",
                            "你问的这个问题我很熟悉，不过需要先确认下你的使用场景哦",
                            "没问题，你尽管问，我会尽量详细地解答"
                        ]
                    },
                    {
                        keys: ["投诉", "不满", "生气", "差评", "服务差"],
                        replies: [
                            "非常抱歉给你带来不好的体验，我马上为你升级处理~",
                            "对不起让你生气了，我一定会帮你解决好这个问题",
                            "实在抱歉，你的反馈我会严肃记录，现在我们一起解决问题好吗？"
                        ]
                    },
                    {
                        keys: ["会员", "VIP", "权益", "积分", "等级", "会员福利"],
                        replies: [
                            "你好呀！会员分为普通/黄金/钻石三个等级，你想了解哪类权益？比如积分兑换/专属折扣~",
                            "会员积分100分可抵1元，还能兑换礼品哦，需要我查你的当前积分吗？",
                            "黄金会员享9折优惠+免费退换货，钻石会员额外有专属客服，你当前是哪个等级呀？"
                        ]
                    },
                    {
                        keys: ["修改订单", "改地址", "换商品", "取消订单", "订单信息"],
                        replies: [
                            "订单发货前可以修改地址/商品哦，告诉我你的订单号，我帮你操作~",
                            "取消订单需要先确认是否已付款，你订单号多少？我看下当前状态",
                            "想换商品型号是吗？提供订单号后，我帮你查是否支持修改（未发货可改）"
                        ]
                    }
                ]
            };
            ...
            function getSmartReply(message) {
                // 优先匹配关键词
                for (const group of replyCorpus.keywords) {
                    if (group.keys.some(key => message.toLowerCase().includes(key))) {
                        const randomIdx = Math.floor(Math.random() * group.replies.length);
                        return group.replies[randomIdx];
                    }
                }
                // 无匹配时用通用回复
                const randomIdx = Math.floor(Math.random() * replyCorpus.default.length);
                return replyCorpus.default[randomIdx];
            }
            ...
            function sendMessage() {
                ...
                // 模拟"思考+打字"延迟（1-2.5秒随机）
                const randomDelay = 1000 + Math.random() * 1500;
                setTimeout(() => {
                    const replyMsgEl = document.createElement('div');
                    replyMsgEl.className = 'message received';
                    const replyContent = getSmartReply(message);
                    replyMsgEl.innerHTML = `
                        <div>${replyContent}</div>
                        <div class="message-time">${currentTime}</div>
                    `;
                    messagesContainer.appendChild(replyMsgEl);
                    messagesContainer.scrollTop = messagesContainer.scrollHeight;
                }, randomDelay);
            }
            ...
            chatInput.addEventListener('keypress', function(e) {
                if (e.key === 'Enter' && !e.shiftKey) { // 禁止Shift+Enter换行触发发送
                    e.preventDefault();
                    sendMessage();
                }
            });
